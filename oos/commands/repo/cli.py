...
from oos.common import atomgit
...
def __prepare_local_repo(atomgit_pat, atomgit_email, work_branch,
                         repo_org, repo_name, repo_path, reuse_branch=False):
    git_user, g_email = atomgit.get_user_info(atomgit_pat)
    git_email = atomgit_email or g_email
    if not git_email:
        raise click.ClickException(
            "Your email was not publicized in atomgit, need to manually "
            "specified by -e or --atomgit-email")
    local_repo = PkgGitRepo(atomgit_pat, repo_org,
                            git_user, git_email,
                            repo_name=repo_name)
...
def __find_repo_yaml_file(repo_name, community_path, atomgit_org):
    file_name = repo_name + '.yaml'
    cmd = 'find %(community_path)s -name ' \
          '"%(file_name)s" |grep %(atomgit_org)s' % {
              "community_path": community_path,
              "file_name": file_name,
              "atomgit_org": atomgit_org}
...
@click.option("-t", "--atomgit-pat", envvar='ATOMGIT_PAT', required=True,
              help="AtomGit personal access token")
@click.option("-e", "--atomgit-email", envvar='ATOMGIT_EMAIL',
              help="Email address for git commit changes, automatically "
                   "query from atomgit if you have public in atomgit")
...
def create(repos_file, repo, atomgit_pat, atomgit_email, atomgit_org,
           community_path, work_branch, reuse_branch, do_push):
    community_repo = __prepare_local_repo(
        atomgit_pat, atomgit_email, work_branch,
...
@click.option('-dp', '--do-push', is_flag=True,
              help="Do PUSH or not, if push it will create pr")
def branch_create(repos_file, repo, branches, atomgit_pat, atomgit_email,
                  atomgit_org, community_path, work_branch, reuse_branch, do_push):
    ...
    for repo in repos:
        yaml_file = __find_repo_yaml_file(
            repo, community_repo.repo_dir, atomgit_org)
        if not yaml_file:
            continue
...
@click.option('-dp', '--do-push', is_flag=True,
              help="Do PUSH or not, if push it will create pr")
def branch_delete(repos_file, repo, branch, atomgit_pat, atomgit_email,
                  atomgit_org, community_path, work_branch, reuse_branch,
                  do_push):
    repos = __get_repos(repo, repos_file)
    community_repo = __prepare_local_repo(
        atomgit_pat, atomgit_email, work_branch,
...
@click.option("-o", "--atomgit-org", envvar='ATOMGIT_ORG', required=True,
              ...
              help="AtomGit organization name of openEuler")
...
def pr_comment(atomgit_pat, atomgit_org, projects_data,
               repo, pr, comment):
    ...
    if repo and pr:
        ...
        repo = PkgGitRepo(atomgit_pat, atomgit_org, repo_name=repo)
    ...
    for row in projects_data.itertuples():
        ...
        repo = PkgGitRepo(atomgit_pat, atomgit_org, repo_name=row.repo_name)
...
def fetch_open_pr(atomgit_pat, repos, output):
    if repos is None:
        repos = []
    ...
    results = {}
    for repo in repos:
        atomgit_org = repo.split('/')[0]
        repo_name = repo.split('/')[1]
        repo_obj = PkgGitRepo(atomgit_pat=atomgit_pat, atomgit_org=atomgit_org, repo_name=repo_name)
...
def branch_version_list(repo, atomgit_pat, atomgit_org, suffix, keyword):
    repo_obj = PkgGitRepo(atomgit_pat=atomgit_pat, repo_name=repo, atomgit_org=atomgit_org)
...
def create_pr(atomgit_pat, atomgit_org, remote_branch, add_commit, comment):

    # 这里不获取当前目录的信息 统一放到PkgGitRepo内部 repo_name在后续刷新
    local_repo = PkgGitRepo(atomgit_pat, atomgit_org, repo_name='no_use')
...
def pr_comment(atomgit_pat, atomgit_org, sig, author, comment, number):
    ...
    for info in repo_infos:
        ...
        pos = pr_link.rfind('/') + 1
        num = int(pr_link[pos:])
        repo = PkgGitRepo(atomgit_pat, atomgit_org, repo_name=info['repo'][14:])
